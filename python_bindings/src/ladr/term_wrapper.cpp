...
PYBIND11_MODULE(term, m) {
    m.doc() = "Python bindings for LADR term module (wrapper)";
    ...
    try {
        ...
        py::module combined = py::module::import("ladr_combined.term");
        ...
        std::string err = std::string("Failed to import ladr_combined.term module: ") + e.what();
        m.attr("import_error") = err;
        
        // Define a function to report the error
        m.def("get_import_error", []() {
            throw py::import_error("Failed to import ladr_combined.term module. Make sure it's properly installed.");
        });
    }
} 