...
PYBIND11_MODULE(parse, m) {
    m.doc() = "Python bindings for LADR parse module (wrapper)";
    ...
    try {
        ...
        py::module combined = py::module::import("ladr_combined.parse");
        ...
        std::string err = std::string("Failed to import ladr_combined.parse module: ") + e.what();
        m.attr("import_error") = err;
        
        // Define a function to report the error
        m.def("get_import_error", []() {
            throw py::import_error("Failed to import ladr_combined.parse module. Make sure it's properly installed.");
        });
    }
} 